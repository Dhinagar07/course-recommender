...
export default function HomePage() {
  ...
  const fetchCourses = async (topic = null) => {
    ...
    try {
      let data;
      if (session?.user?.id) {
        let url = `http://127.0.0.1:5000/recommend?user_id=${session.user.id}&k=1000`;
        if (topic) url += `&topic=${encodeURIComponent(topic)}`; // add topic to CB fetch
  ...
  const applyKbFilters = (filters) => {
    ...
    const maxSubscribers =
      Math.max(...filtered.map((c) => c.num_subscribers || 0)) || 1;
    const maxReviews =
      Math.max(...filtered.map((c) => c.num_reviews || 0)) || 1;
    const maxLectures =
      Math.max(...filtered.map((c) => c.num_lectures || 0)) || 1;
  ...
  // Handle topic search submit
  const handleSearch = async (e) => {
    e.preventDefault();
    if (!searchTopic.trim()) return;
    await fetchCourses(searchTopic); // fetch CB recommendations for topic
  };
